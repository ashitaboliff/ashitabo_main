...
import { getUserRoleAction, getAllUserDetailsAction } from '@/features/admin/components/action' // Added getAllUserDetailsAction
...
const Page = async ({ searchParams }: AdminUsersPageProps) => {
	...
	if (!session?.user?.id) { // Check for session.user.id as well
		return notFound()
	}
  ...
  const currentPageString = Array.isArray(adminUserPageParam) ? adminUserPageParam[0] : adminUserPageParam;
  ...
  const usersPerPageString = Array.isArray(adminUserLimitParam) ? adminUserLimitParam[0] : adminUserLimitParam;
  ...
  if (typeof adminUserSortParam === 'string' && (adminUserSortParam === 'new' || adminUserSortParam === 'old')) {
    sort = adminUserSortParam;
  } else if (Array.isArray(adminUserSortParam) && typeof adminUserSortParam[0] === 'string' && (adminUserSortParam[0] === 'new' || adminUserSortParam[0] === 'old')) {
