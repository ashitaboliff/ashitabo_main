...
import { adminRevalidateTagAction, createPadLockAction, deletePadLockAction } from './action'
...
const PadLockEdit = ({
  ...
  const [popupData, setPopupData] = useState<PadLock | undefined | null>(padLocks?.[0] ?? undefined)
  ...
  const currentPadLocks = padLocks?.slice(indexOfFirstPadLock, indexOfLastPadLock) ?? []
  ...
  const onSubmit = async (data: any) => {
    ...
    const res = await createPadLockAction({ name, password: password.toString() })
    if (res.status === 201) {
          ...
          <tbody>
            {currentPadLocks.map((padLock) => (
              ...
              >
                ...
                <td>{padLock.name}</td>
                <td>{format(padLock.createdAt, 'yyyy年MM月dd日hh時mm分ss秒', { locale: ja })}</td>
                <td>{format(padLock.updatedAt, 'yyyy年MM月dd日hh時mm分ss秒', { locale: ja })}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      >
        <div className="flex flex-col items-center space-y-2 text-sm">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <div>{popupData?.createdAt && format(popupData?.createdAt, 'yyyy年MM月dd日hh時mm分ss秒', { locale: ja })}</div>
            <div className="font-bold">更新日:</div>
            <div>{popupData?.updatedAt && format(popupData?.updatedAt, 'yyyy年MM月dd日hh時mm分ss秒', { locale: ja })}</div>
          </div>
